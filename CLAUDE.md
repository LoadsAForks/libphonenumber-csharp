...
- `resources/` — XML metadata (`PhoneNumberMetadata.xml`, `ShortNumberMetadata.xml`, `PhoneNumberAlternateFormats.xml`, `PhoneNumberMetadataForTesting.xml`), plus `geocoding/`, `carrier/`, `timezones/`. **These are copied verbatim from upstream** — do not hand-edit. The library reads XML directly (not protobuf — see `csharp/README.md`).
...
**Tests require `geocoding.zip` and `testgeocoding.zip` to exist as embedded resources.** Generate them once before running tests:
...
- **Singleton + metadata loading.** `PhoneNumberUtil.GetInstance()` is the entry point. Region/country metadata is lazily loaded by `MetadataManager` from the XML files embedded as resources by `PhoneNumbers.csproj`. `BuildMetadataFromXml.cs` parses the XML into the `Phonemetadata` object graph at runtime — there is no protobuf step in C#.
